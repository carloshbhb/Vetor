...
async function generateReviewVideoJob(review: {
  ...
  try {
    ...
    const videoId = await createVideoQueueEntry({
      ...
      product_url: review.affiliate_url || `https://www.vetor.blog/reviews/${review.slug}`,
      type: 'review',
      slug: review.slug,
...
async function generateComparisonVideoJob(article: {
  ...
  try {
    ...
    const videoId = await createVideoQueueEntry({
      ...
      product_url: firstProduct.product_url || `https://www.vetor.blog/comparativos/${article.slug}`,
      type: 'comparison',
      slug: article.slug,
      script_hook: script.hook,
      script_text: JSON.stringify(script),
