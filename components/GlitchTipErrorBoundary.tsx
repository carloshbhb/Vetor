...
interface GlitchTipErrorBoundaryProps {
  children: React.ReactNode;
  fallback?: React.ReactNode;
  showDialog?: boolean;
}

interface GlitchTipErrorBoundaryState {
  hasError: boolean;
  error?: Error;
  errorInfo?: React.ErrorInfo;
}

export default class GlitchTipErrorBoundary extends React.Component<
  GlitchTipErrorBoundaryProps,
  GlitchTipErrorBoundaryState
> {
  constructor(props: GlitchTipErrorBoundaryProps) {
    super(props);
    this.state = { hasError: false };
  }

  static getDerivedStateFromError(error: Error): GlitchTipErrorBoundaryState {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('[GlitchTip ErrorBoundary]', error, errorInfo);
    }
  }
  ...
  handleReportFeedback = () => {
    if (typeof window !== 'undefined' && window.Sentry && window.Sentry.showReportDialog) {
      // Open GlitchTip user feedback dialog
