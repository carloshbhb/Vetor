...
export const HorizontalCTA: React.FC<HorizontalCTAProps> = ({ section, palette }) => {
  ...
  const entranceProgress = interpolate(frame, [0, 20], [0, 1], { extrapolateRight: "clamp" });
  ...
  const productOpacity = interpolate(frame, [8, 22], [0, 1], { extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill>
      {hasImage ? (
        <AbsoluteFill>
          <Img src={getImageSrc(visual.backgroundImage)} style={{ width: "100%", height: "100%", objectFit: "cover", filter: "blur(30px) brightness(0.2) saturate(0.5)", transform: `scale(${kenBurnsScale})` }} />
      ...
      <AbsoluteFill style={{ opacity: entranceProgress, flexDirection: "column", justifyContent: "flex-start", alignItems: "center", paddingTop: 80 }}>
        <div style={{ marginBottom: 30, transform: `translateY(${interpolate(entranceProgress, [0, 1], [30, 0])}px)` }}>
        ...
        {hasImage && (
          <div style={{ position: "relative", marginBottom: 25, opacity: staggerDelay(0), transform: `scale(${productScale})` }}>
        ...
        {visual.urgency.enabled && (
          <div style={{ marginBottom: 25, opacity: staggerDelay(1), transform: `translateY(${interpolate(staggerDelay(1), [0, 1], [20, 0])}px)` }}>
        ...
        {visual.qr_code.enabled && (
          <div style={{ marginBottom: 20, opacity: staggerDelay(2), transform: `scale(${interpolate(staggerDelay(2), [0, 1], [0.8, 1])})` }}>
            <QRCodeAnimated url={visual.qr_code.url} size={visual.qr_code.size} delay={visual.qr_code.animation.delay} />
          </div>
        )}
        <div style={{ marginBottom: 25, opacity: staggerDelay(3), transform: `scale(${interpolate(staggerDelay(3), [0, 1], [0.7, 1])})` }}>
        ...
        <div style={{ opacity: staggerDelay(4) }}>
