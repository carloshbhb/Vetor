...
export interface SEACheckResult {
  failedChecks: string[];
  allPassed: boolean;
}

// ─── SEO Checks (mirrors ReviewForm.tsx logic) ─────────────────────────────
export function runSeoChecks(data: any): SEACheckResult {
...
// ─── Main generate function (shared by /api/generate and autonomous-agent) ──
export async function generateReview(input: GenerateInput): Promise<GenerateResult> {
