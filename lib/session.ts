// Sessoes HMAC-SHA256 sem dependencias Node (Edge-runtime safe).
// Usa SHA-256/HMAC implementados em JS puro para funcionar no middleware (Edge),
// que nao suporta o modulo 'crypto' do Node.js.
// O formato e identico ao anterior: `${value}.${hmacSha256Hex(value, secret)}`,
// entao cookies ja emitidos continuam validos.
...
function hmacHex(value: string, secret: string): string {
  return toHex(hmacSha256(encoder.encode(secret), encoder.encode(value)));
}

// Comparacao em tempo constante (sem Buffer/timingSafeEqual do Node)
